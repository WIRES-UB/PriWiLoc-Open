...
def get_n_ap_from_path(data_path: str) -> int:
    """Find the number of access points (APs) from the given data path.

    Args:
        data_path: Path to the data file.

    Returns:
        Number of access points (APs) in the dataset.
    """
    assert os.path.exists(data_path), f"Data file csv {data_path} does not exist."

    parent_dir = os.path.join(os.path.dirname(data_path), "features_aoa")
    ap_file_path = os.path.join(parent_dir, "ap.h5")
    ap_aoas, _ = read_ap_file(ap_file_path)
    return len(ap_aoas)

def read_ap_file(ap_file_path):
    if not os.path.exists(ap_file_path):
        raise FileNotFoundError(f"'ap.h5' file not found in the directory: {ap_file_path}")
    with h5py.File(ap_file_path, "r") as f:
            ap_aoas = np.array(f.get(DatasetKeys.AP_AOAS.value), dtype=np.float32).squeeze()
            ap_locs = np.transpose(np.array(f.get(DatasetKeys.AP_LOCATIONS.value), dtype=np.float32))

    return ap_aoas, ap_locs
...
def check_git_uncommitted_changes_ignore_untracked():
    """
    Check for uncommitted changes in the current git repository, ignoring untracked files.
    If there are changes (except untracked), print an error and exit.
    """
    status = subprocess.run(['git', 'status', '--porcelain'], capture_output=True, text=True)
    lines = status.stdout.splitlines()
    # Ignore lines that start with '??' (untracked files)
    changes = [line for line in lines if not line.startswith('??')]
    if changes:
        print(
            f"Error: There are uncommitted changes (excluding untracked files).\n"
            f"Please commit or stash them before proceeding.\n"
            f"Do not directly commit to master unless you are sure such change should live on master.\n"
            f"Check with git status.\n"
            f"Create a separate branch with:\n"
            f"    git checkout -b <branch_name>"
        )
        sys.exit(1)


def check_git_sync_with_remote():
    """
    Check if the current git branch is in sync with the remote branch.
    If not, print an error and exit.
    Also error if the branch is not uploaded to remote.
    """
    ...
    # Get local HEAD hash
    local_hash_proc = subprocess.run(['git', 'rev-parse', 'HEAD'], capture_output=True, text=True)
    local_hash = local_hash_proc.stdout.strip()

    # Get remote branch hash
    remote_hash_proc = subprocess.run(['git', 'rev-parse', f'origin/{branch}'], capture_output=True, text=True)
    remote_hash = remote_hash_proc.stdout.strip()

    # If remote branch does not exist, returncode will be non-zero
    if remote_hash_proc.returncode != 0:
        print(f"Error: The branch '{branch}' has not been pushed to remote (origin).")
        sys.exit(1)

    # Check sync status
    if local_hash != remote_hash:
        print(
            f"Error: Local HEAD ({local_hash}) does not match origin/{branch} ({remote_hash}).\n"
            f"Please check with git status. Commit and push your changes if needed."
        )
        sys.exit(1)


def git_check():
    """Check for uncommitted changes and if the branch is in sync with remote."""
    check_git_uncommitted_changes_ignore_untracked()
    check_git_sync_with_remote()
